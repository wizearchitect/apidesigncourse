...
app.get('/api/states',
	(req, resp) => {
		...
		// Send back - Content-Type: application/json
		resp.type('application/json')

		db.findAllStates()   //this is javascript
		.then (result => {
			//200 OK
			resp.status(200)
...
app.get('/api/state/:state',
	(req,resp) => {
		const stateAbbrev = req.params.state;   // to get state

		db.findCitiesByState(stateAbbrev)
			.then(result => {
				//200 OK
				resp.status(200)
				//resp.json(result);
